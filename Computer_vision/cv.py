...
def emnist_train(model):
    ...
    y_test = np.reshape(X_test, (X_test.shape[0], 28, 28, 1))
    ...
    xtc = keras.utils.to_categorical(y_train, len(emnist_labels))
    ytc = keras.utils.to_categorical(y_test, len(emnist_labels))
    ...
    model.fit(x_train, xtc, validation_data=(x_test, ytc), callbacks=[learning_rate_reduction], batch_size=64, epochs=30)
...
def emnist_predict_img(model, img):
    ...
    return chr(emnist_labels[res[0]])
...
if __name__ == "__main__":
    ...
    text = img_to_str(model, "test.png")
    print(text)
